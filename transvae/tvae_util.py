...
def tokenizer_peptides(peptide):
    "Tokenizes peptides string"
...
def get_char_weights(train_smiles, params, freq_penalty=0.5):
    ...
    for smile in tqdm(train_smiles):
        ...
        for j in range(i, params['MAX_LENGTH']):
            char_dist['_'] += 1
    for i, v in enumerate(tqdm(char_dist.values())):
...
# def get_char_weights(train_sequences, params, freq_penalty=0.5, sequence_type='smiles'):
#     """
#     Calculates token weights for a set of input data.
#     Args:
#         train_sequences (list): List of training sequences (e.g., SMILES or amino acid sequences).
#         params (dict): Dictionary containing parameters such as 'NUM_CHAR', 'MAX_LENGTH', and 'CHAR_DICT'.
#         freq_penalty (float): The minimum scaling factor for character frequencies.
#         sequence_type (str): Type of sequences provided ('smiles' or 'amino_acid').
#     Returns:
#         np.array: Array of character weights.
#     """
#     char_dist = {k: 0 for k in params['CHAR_DICT'].keys()}
#     char_counts = np.zeros((params['NUM_CHAR'],))
#     char_weights = np.zeros((params['NUM_CHAR'],))
#
#     # Calculate character distributions
#     for sequence in tqdm(train_sequences):
#         for i, char in enumerate(sequence):
#             char_dist[char] += 1
#         for j in range(i, params['MAX_LENGTH']):
#             char_dist['_'] += 1  # Padding character
#
#     for i, v in enumerate(tqdm(char_dist.values())):
#         char_counts[i] = v
#
#     if sequence_type == 'sequences':
#         # If it's an amino acid sequence, assign equal weights
#         char_weights.fill(1.0)
#     else:
#         # Otherwise, calculate weights based on frequency
#         top = np.sum(np.log(char_counts))
#         for i in range(char_counts.shape[0]):
#             char_weights[i] = top / np.log(char_counts[i])
#
...
#         scaler = MinMaxScaler([freq_penalty, 1.0])
#         char_weights = scaler.fit_transform(char_weights.reshape(-1, 1))
#         char_weights = char_weights.flatten()  # Ensure the array is flattened
#
#     return char_weights

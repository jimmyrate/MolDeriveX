...
from transvae.data import vae_data_gen, make_std_mask,vae_data_gen_list
...
class VAEShell():
    ...
    def train(self, train_mols, val_mols, train_props=None, val_props=None,
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for j, data in enumerate(tqdm(train_iter)):
                avg_losses = []
                avg_bce_losses = []
                avg_bcemask_losses = []
                avg_kld_losses = []
    ...
    def reconstruct_encoder(self, data, log=True):
        # data = vae_data_gen(data, props=None, char_dict=self.params['CHAR_DICT'])
        data = vae_data_gen_list(data,  char_dict=self.params['CHAR_DICT'])
        ...
        self.model.eval()
        src_masks = []
        hidden_states = torch.empty((data.shape[0], self.params['d_latent'])).cpu()
        ...
        for j, data in enumerate(data_iter):
            ...
            hidden_states[j*self.batch_size:(j+1)*self.batch_size, :] = mem.detach().cpu()

        return hidden_states,src_masks



    def reconstruct_decoder(self, hidden_states, src_masks=None, method='greedy', return_str=True):
        decoded_smiles = []
        self.model.eval()
        # self.batch_size = self.params['BATCH_SIZE']
        self.batch_size = 100
        ...
        # for mem in tqdm(hidden_states):
        for i, mem in enumerate(hidden_states.split(self.batch_size)):
        # for i, mem in enumerate(tqdm(hidden_states.split(self.batch_size))):
            # src_mask = src_masks[i]  # 获取对应批次的src_mask
            # src_mask = src_mask.to('cuda:0')
            mem = mem.to('cuda:0')
            if method == 'greedy':
                decoded = self.greedy_decode(mem, src_mask=None)
            else:
                decoded = None
        ...
        #     if return_str:
        #         decoded = decode_mols(decoded, self.params['ORG_DICT'])
        #         # 检查每个解码的SMILES字符串是否代表一个合法的分子
        #         valid_decoded = []
        #         for smi in decoded:
        #             mol = Chem.MolFromSmiles(smi)
        #             if mol is None:  # 如果不是合法的分子
        #                 smi = random.choice(simple_molecules)  # 从简单分子中随机选择一个
        #             valid_decoded.append(smi)
        #         decoded_smiles += valid_decoded
        #     else:
        #         decoded_smiles.append(decoded)
...
class VAEEncoder(nn.Module):
    ...
    def __init__(self, layer, N, d_latent, bypass_bottleneck, eps_scale):
        ...
        # self.z_means, self.z_var = nn.Linear(64, d_latent), nn.Linear(64, d_latent)
...
class VAEDecoder(nn.Module):
    ...
    def __init__(self, encoder_layers, decoder_layers, N, d_latent, bypass_bottleneck):
        ...
        # self.linear = nn.Linear(d_latent, 64)

...
def vae_data_gen_list(mols, char_dict):
    """
...
def vae_data_gen(mols, props, char_dict):
    ...
    peptides = mols[:,0]
    if props is None:
        props = np.zeros(peptides.shape)
    del mols
    peptides = [tokenizer_peptides(x) for x in peptides]
    encoded_data = torch.empty((len(peptides),128))
    for j, smi in enumerate(tqdm(peptides)):
        encoded_smi = encode_smiles(smi, 126, char_dict)
        encoded_smi = [0] + encoded_smi
        encoded_data[j,:-1] = torch.tensor(encoded_smi)
        encoded_data[j,-1] = torch.tensor(props[j])

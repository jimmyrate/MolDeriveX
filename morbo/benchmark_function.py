...
class BenchmarkFunction(Module):
    ...
    def _evaluate_base_f(self, X: Tensor) -> Tuple[Any, Any]:
        Y, smiles = self.base_f(X)
        Y = Y.to(**self.tkwargs)
    ...
    def forward(self, X: Tensor) -> Tuple[Any, Any]:
        ...
        noise_free_eval, smiles = self._evaluate_base_f(X)
        self.Y_noise_free = self.Y_noise_free.to(device)
        noise_free_eval = noise_free_eval.to(device)
        self.Y_noise_free = torch.cat([self.Y_noise_free, noise_free_eval], dim=0)

        self.observation_noise_std = self.observation_noise_std.to(device)
        self.observation_noise_bias = self.observation_noise_bias.to(device)
        return (
            noise_free_eval

...
from morbo.utils import get_fitted_model, coalesce, decay_function,get_indices_in_hypercube,get_indices_closest_hypercube
from scipy.stats.mstats import winsorize
from torch import Tensor
from torch.nn import Module, ModuleList
from morbo.problems.mol_ren import reinitialize_model,check_for_nan,replace_nan,remove_nan
...
class TRBOState(Module):
    ...
    def update(
        ...
        # print('X_history shape is', self.X_history.shape)
        # self.X_history, indices = torch.unique(self.X_history, return_inverse=True, dim=0)
        # self.Y_history = self.Y_history[indices]
        # print('X_history_clean shape is',self.X_history.shape)
        #0425
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.TR_index_history = self.TR_index_history.to(device)
        new_ind = new_ind.to(device)


        self.TR_index_history = torch.cat([self.TR_index_history, new_ind], dim=0)
        # self.TR_index_history = self.TR_index_history[indices]
    ...
    def log_restart_points(self, X: Tensor, Y: Tensor) -> None:
        ...
        #0425
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self._restart_X = self._restart_X.to(device)
        X = X.to(device)
        self._restart_Y = self._restart_Y.to(device)
        Y = Y.to(device)
    ...
    def set_pslmodel(self,optimizer,pslmodel):
        self.pslmodel = pslmodel
        self.optimizer = optimizer


    # def update_pslmodel(self):
    #     for _ in range(self.tr_hparams.n_steps):
    #         self.pslmodel.train()
    #         alpha = np.ones(self.num_objectives)
    #         pref = np.random.dirichlet(alpha,self.tr_hparams.n_pref_update)
    #         pref_vec  = torch.tensor(pref).to('cuda').float() + 0.0001
    #         x = self.pslmodel(pref_vec).detach()
    #         x_indice = get_indices_closest_hypercube(self,x)
    #         grad = torch.zeros_like(x)
    #         for i in range(self.tr_hparams.n_trust_regions):
    #             tr = self.trust_regions[i]
    #             init_grad = torch.zeros_like(x).to(torch.float64)
    #             inds_in_tr = get_indices_in_hypercube(tr.X_center_normalized,x, length=tr.length)
    #             if inds_in_tr.shape[0] != 0:
    #                 x_indice[inds_in_tr] = i
    #             inds_in_tr = (x_indice==i)
    #             current_grad = tr.update_grad(x,self.utopia,pref_vec,self.tr_hparams.n_pref_update)
    #             init_grad[inds_in_tr] = current_grad[inds_in_tr]
    #             grad += init_grad
    #         grad = grad
    #
    #         self.optimizer.zero_grad()
    #         self.pslmodel(pref_vec).backward(grad)
    #         torch.nn.utils.clip_grad_norm_(self.pslmodel.parameters(),max_norm = 0.1)
    #
    #         # print("Checking gradients post-update...")
    #         # if any(check_for_nan(param.grad, f"{name} gradient") for name, param in self.pslmodel.named_parameters() if
    #         #        param.grad is not None):
    #         #     # print("NaN detected in gradients, apply reinitialize model")
    #         #     reinitialize_model(self.pslmodel)
    #         #     continue
    #
    #         # print("Checking gradients post-update...")
    #         # if any(check_for_nan(param.grad) for name, param in self.pslmodel.named_parameters() if
    #         #        param.grad is not None):
    #         #     # print("NaN detected in gradients, applying fixes...")
    #         #     for name, param in self.pslmodel.named_parameters():
    #         #         if param.grad is not None and torch.isnan(param.grad).any():
    #         #             param.grad = replace_nan(param.grad, 0.0)
    #         self.optimizer.step()
    #         self.scheduler.step()
    #
    #     self.pslmodel.eval()
    ...
    def update_pslmodel(self):
        ...
        for _ in range(self.tr_hparams.n_steps):
            ...
            for i in range(self.tr_hparams.n_trust_regions):
                ...
                if torch.isnan(current_grad).any():
                    count = True
                    # current_grad = replace_nan(current_grad, 0.0884)
                init_grad[inds_in_tr] = current_grad[inds_in_tr]
                grad += init_grad
            grad = grad
            ...
            self.optimizer.zero_grad()
            self.pslmodel(pref_vec).backward(grad)
            torch.nn.utils.clip_grad_norm_(self.pslmodel.parameters(), max_norm=3)

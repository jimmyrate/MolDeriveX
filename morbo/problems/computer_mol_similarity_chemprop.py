...
def read_smiles_from_csv(file_path):
    """
    从CSV文件中读取名为'SMILES'的列，并将其保存为列表。

    :param file_path: CSV文件的路径。
    :return: 包含SMILES字符串的列表。
    """
    try:
        # 读取CSV文件
        ...
        if 'SMILES' in df.columns:
            # 提取SMILES字符串并将它们作为列表返回
            return df['SMILES'].tolist()
        else:
            print("错误：CSV文件中没有找到'SMILES'列。")
            return None
...
def save_list_to_csv(data, file_path):
    ...
    try:
        # 将多维列表转换为DataFrame
        df = pd.DataFrame(data, columns=['SMILES', 'Similarity', 'Activity','Chem_Activity'])
...
def calculate_tanimoto(molecule1, molecule2):
    # 通过SMILES字符串创建分子对象
    ...
    # 计算Tanimoto系数
    return FingerprintSimilarity(fingerprint1, fingerprint2)


def calculate_active(gen_molecule):
    model_path = "/root/morbo/morbo/problems/checkpoint-6510"
    ...
    with torch.no_grad():
        ...
        probabilities = torch.nn.functional.softmax(logits, dim=1)[:, 1].numpy()
    active = float(probabilities)
    return active
...
# def calculate_active_gpu(gen_molecule):
#     model_path = "/root/morbo/morbo/problems/checkpoint-6430"
#
#     # 确保GPU可用
...
#     # 准备数据
#     tokenized_samples = tokenizer(gen_molecule, padding="max_length", truncation=True, max_length=128, return_tensors="pt")
#     tokenized_samples = tokenized_samples.to(device)
#
#     # 计算不需要梯度
#     with torch.no_grad():
#         outputs = model(**tokenized_samples)
...
#
#     # 将结果转换为CPU，然后转换为numpy
#     active = float(probabilities.cpu().numpy())
#     return active


def calculate_similarity_and_activity(training_data, generated_data):
    result_matrix = []

    for gen_molecule in tqdm(generated_data):
        activity = calculate_chemprop_active(args=PredictArgs().parse_args(), gen_molecule=[gen_molecule])
        similarity = max(calculate_tanimoto(gen_molecule, train_molecule) for train_molecule in training_data)
        # activity = calculate_active(gen_molecule)

        # result_matrix.append([gen_molecule,similarity, activity, chem_active])
        result_matrix.append([gen_molecule,similarity, activity])
    return np.array(result_matrix)

def calculate_similarity_and_remove_duplicates(training_data, generated_data):
    non_duplicate_data = []

    for gen_molecule in tqdm(generated_data):
        # 计算最大相似度
        max_similarity = max(calculate_tanimoto(gen_molecule, train_molecule) for train_molecule in training_data)

        if max_similarity < 1:
            non_duplicate_data.append(gen_molecule)
...
train_path = '/root/morbo/TransVAE-master/data/Gram-positive-for-mbo.csv'
generated_path = '/root/morbo/TransVAE-master/data/Gram-positive-for-mbo.csv'
#
#
training_data = read_smiles_from_csv(train_path)
generated_data = read_smiles_from_csv(generated_path)
#
similar_molecules = calculate_similarity_and_activity(training_data, generated_data)
print(f"Similar Molecules: {similar_molecules}")
# filtered_generated_data = calculate_similarity_and_remove_duplicates(training_data, generated_data)
# save_list_to_csv(similar_molecules, '/home/leo/fightinglee/Antibiotic-project/data/train_2024_1_4/result/positive_result_active.csv')


# file_path = '/home/leo/fightinglee/Antibiotic-project/data/train_2024_1_4/negative_activity_results.csv'  # 替换为您的文件路径
# df = pd.read_csv(file_path)
#
# # 筛选出Similarity小于1的行
# filtered_df = df[df['Activity'] < 0.5]
# # filtered_df = filtered_df[filtered_df['Activity'] < 0.5]
# # 只保留SMILES这一列
# smiles_df = filtered_df[['SMILES']]
#
# # 保存为新的CSV文件
# new_file_path = '/home/leo/fightinglee/Antibiotic-project/data/train_2024_1_4/negative_data_A<0.5.csv'  # 您想要保存的新文件路径
# smiles_df.to_csv(new_file_path, index=False)
...
class TrustRegion(ABC, Module):
    ...
    def _check_for_updates_to_model_training_data(self) -> bool:
        ...
        if torch.equal(self.model_training_data[0], self.X) and torch.equal(
            ...
            return False
        return True
    def update_grad(self, x, z, pref_vec ,n_pref_update)->None:
        r"""
        Returns:
            the gradient of current trs.
        """
        coef_lcb = 0.5
        data = x.clone().detach()
        # data = unnormalize(data,self.bounds)
        # data = normalize(data, self.bounds)

        data.requires_grad_(requires_grad = True)
        mean_grad_total = torch.empty([n_pref_update,0,data.shape[1]]).to(torch.float64).to('cuda')
        variance_grad_total = torch.empty([n_pref_update,0,data.shape[1]]).to(torch.float64).to('cuda')

        # mean_grad_total = torch.empty([n_pref_update, 0, data.shape[1]]).to('cuda')
        # variance_grad_total = torch.empty([n_pref_update, 0, data.shape[1]]).to('cuda')

        posterior = self.model.posterior(data)
        total_mean = posterior.mean
        total_variance = posterior.variance
        for i, model in enumerate(self.model.models):
            curr_posterior = model.posterior(data)
            mean = curr_posterior.mean
            variance = curr_posterior.variance

            #compute gradient
            mean.backward(torch.ones_like(mean),retain_graph = True)
            mean_grad = data.grad.clone().unsqueeze(1)
            mean_grad_total = torch.cat([mean_grad_total,mean_grad],dim = 1)
            data.grad.zero_()

            variance.sum().backward(retain_graph = False)
            variance_grad = data.grad.clone().unsqueeze(1)
            variance_grad_total = torch.cat([variance_grad_total,variance_grad],dim = 1)
            data.grad.zero_()
        

        value = total_mean - coef_lcb * total_variance
        value_grad = mean_grad_total - coef_lcb * variance_grad_total
        tch_idx = torch.argmax((1/pref_vec) * torch.abs(z-value), axis = 1)
        ...
        preference_grad = torch.sum((1 - value * (1 / pref_vec))**2,dim = 1).view(-1,1)
        tch_grad = tch_grad + preference_grad
    ...
    def update_model(self) -> bool:
        ...
        if self._check_for_updates_to_model_training_data():
            ...
            # 把数据做2次缩放，再次缩放到中心点的TR范围内
            #     intf2 = Normalize(d=self.dim, bounds=self.get_bounds(model_space=True))
            #     intf = ChainedInputTransform(intf1=intf, intf2=intf2)
            # Standardize Y
            #print(torch.any(torch.isnan(self.Y)).item())

            # unique_Y, indices = torch.unique(self.Y, dim=0, return_inverse=True)
            # _, first_indices = torch.unique(indices, sorted=True, return_inverse=True)
            # self.Y = unique_Y
            # self.X = self.X[first_indices]

            # Apply Winsorizing to Y to limit extreme values which can affect the GP model.
            ...
            # Standardize Y to have zero mean and unit variance, typically improves GP training.
            ...
            if update_streaks:
                ...
                if self.n_successes >= self.tr_hparams.success_streak:
                    ...
                    if self.length < self.tr_hparams.length_min:
                        if self.length < self.tr_hparams.length_restart:
                            self.length.fill_(self.tr_hparams.length_init)
                        return True
                # elif self.n_failures < self.tr_hparams.failure_streak and \
                #         self.n_successes < self.tr_hparams.success_streak:
                #     self._reset_counters()
    ...
    def _update_training_data(self, X_all: Tensor, Y_all: Tensor) -> None:
        ...
        if tr_indices.numel() < 2:
            device = tr_indices.device
            num_rows = X_all.size(0)  # 获取 X_all 的行数
            num_to_select = max(1, int(0.1 * num_rows))  # 计算要选择的 10% 的行数，至少选择 1 行

            # 随机选择 10% 的行索引
            available_indices = torch.arange(num_rows)
            available_indices = available_indices[~torch.isin(available_indices.to(device), tr_indices)]

            # 从剩余的行中随机选择索引
            random_indices = torch.randperm(available_indices.size(0))[:num_to_select]
            selected_indices = available_indices[random_indices]

            # 将 tr_indices 添加到选中的索引中，并去重
            final_indices = torch.cat((selected_indices.to(device), tr_indices)).unique()

            # 根据最终的行索引更新 self.X 和 self.Y
            self.X = X_all[final_indices]
            self.Y = Y_all[final_indices]
        else:

...
from gpytorch.settings import use_toeplitz, fast_pred_var, max_preconditioner_size, max_root_decomposition_size
...
def get_indices_closest_hypercube(state,X):
    n = len(state.trust_regions)
    total_distance = torch.empty([X.shape[0],0]).to('cuda')
    for i in range(n):
        tr = state.trust_regions[i]
        X_norm = normalize(X, bounds=tr.bounds)
        #distance =torch.sum(torch.abs(X -tr.X_center_normalized),dim = -1).view(-1,1)
        #distance = torch.norm(X-tr.X_center_normalized, p = 2,dim = 1).view(-1,1)
        distance = torch.norm(X_norm-tr.X_center_normalized, p = 2,dim = 1).view(-1,1)
        total_distance = torch.cat([total_distance,distance],dim = -1)  
    return torch.argmin(total_distance,dim = -1)


def apply_transform_and_filter_invalid(
        X: Tensor,
        input_transform: Optional[InputTransform] = None,
) -> Tensor:
    """
    Applies input_transform to X, filters out rows containing NaN or values not in the range [0, 1], and returns the filtered data.

    Args:
        X (Tensor): Input tensor to be transformed.
        input_transform (Optional[InputTransform]): An optional input transformation.

    Returns:
        Tensor: A boolean mask indicating valid rows.
    """
    if input_transform is not None:
        with torch.no_grad():
            X_transformed = input_transform(X)
    else:
        X_transformed = X

    # Check for NaN values
    nan_mask = ~torch.isnan(X_transformed).any(dim=1)

    # Check for values within [0, 1]
    within_bounds_mask = (X_transformed >= 0).all(dim=1) & (X_transformed <= 1).all(dim=1)

    # Combine masks to find valid indices
    valid_indices = nan_mask & within_bounds_mask
...
def get_fitted_model(
    ...
    # valid_indices = apply_transform_and_filter_invalid(X=X,  input_transform=input_transform)
    # X = X[valid_indices]
    # Y = Y[valid_indices]
    # print('Shape of X_fit:', X.shape)

    # print('Shape of self.Y:', Y_all.shape)
    unique_X, unique_indices = torch.unique(X, dim=0, return_inverse=True)
    first_indices = torch.as_tensor(
        [np.where(unique_indices.cpu().numpy() == i)[0][0] for i in range(len(unique_X))],
        dtype=torch.long
    )
    X = unique_X
    Y = Y[first_indices]
    ...
    # with use_toeplitz(False), fast_pred_var(False), \
    #         max_preconditioner_size(10), max_root_decomposition_size(30), \
    #         gpytorch_settings.min_preconditioning_size(0),\
    #         gpytorch_settings.fast_computations(log_prob=use_fast_mvms,
